	...
	if ( !$updated && !empty($_POST['sidebar']) ) {
		...
		foreach ( (array) $_POST[shaken_SSA_ID_BASE] as $widget_number => $ssa_instance )
		{
			...
			$options[$widget_number] =
				array
				(
					'title' => esc_html( $ssa_instance['title'] ),
...
function shaken_sidebar_ads_register() {
	...
	$widget_ops = array('classname' => 'shaken_sidebar_ads', 'description' => __('Configure and display up to four 150x50 sidebar advertisements.', 'shaken'));
	$control_ops = array('id_base' => shaken_SSA_ID_BASE, 'width'=>350);
	$name = __('Shaken - Simple Sidebar Ads', 'shaken');
